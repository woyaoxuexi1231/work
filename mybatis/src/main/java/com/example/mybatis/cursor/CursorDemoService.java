...
public class CursorDemoService {
     ...
     *
     * 【重要】游标查询必须手动管理 SqlSession！
     * 因为游标需要保持数据库连接打开，而 Spring 自动管理的 SqlSession 会在查询后立即关闭。
     * 所以必须使用 SqlSessionFactory.openSession() 手动管理。
     */
    public void demo1_cursorObject() {
        log.info("========== 演示1：使用Cursor对象遍历 ==========");

        // 【关键】手动打开 SqlSession，确保连接不会提前关闭
     ...
     * 适用于需要对每行数据进行复杂处理的场景
     *
     * 【注意】ResultHandler 不需要手动管理 SqlSession
     * 因为 ResultHandler 是在查询过程中逐行回调的，查询完成后数据已经处理完毕
     */
    public void demo2_resultHandler() {
        ...
        // ResultHandler 会在查询过程中逐行回调，不需要手动管理 SqlSession
