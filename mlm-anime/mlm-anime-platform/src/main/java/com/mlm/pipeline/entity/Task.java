 ...
 * 任务提交给厂商后进入 PROCESSING 状态，由
 * {@link com.mlm.pipeline.scheduler.TaskPollingScheduler} 定时轮询厂商接口
 * 获取结果，成功/失败后更新状态并检查所属步骤是否全部完成。
 * <p>
 * 【关键字段说明】
 * <ul>
 *   <li>vendorTaskId — 厂商侧的任务 ID（由 submit 接口返回）</li>
 *   <li>status — 任务状态（int 码：-1=失败, 0=待处理, 1=成功, 2=处理中）</li>
 *   <li>requestJson — 提交给厂商的完整请求参数 JSON</li>
 *   <li>resultJson — 厂商返回的结果 JSON</li>
 *   <li>nextPollAt — 下次可轮询时间，用于避退控制</li>
 * </ul>
 *
 * @author mlm
...
public class Task {
     ...
     * 计算下次轮询时间
     * <p>
     * 将 nextPollAt 设置为 {@code 当前时间 + intervalSeconds}，
     * 用于实现简单的固定间隔避退。
     *
     * @param intervalSeconds 轮询间隔（秒）
     */
