...
# 用法：chmod +x 02-mirrored-queue.sh && ./02-mirrored-queue.sh
# ============================================================================
set -euo pipefail

NODES=("rabbitmq-node1" "rabbitmq-node2" "rabbitmq-node3")
MGMT_PORTS=(15672 15673 15674)
...
echo ""
echo "[1/6] 检查集群是否就绪..."
if ! docker exec rabbitmq-node1 rabbitmqctl cluster_status 2>/dev/null | grep -q "rabbit@rabbitmq-node"; then
...
# 先清理旧策略
docker exec rabbitmq-node1 rabbitmqctl clear_policy mirrored-all 2>/dev/null || true
...
# 使用 rabbitmqadmin 声明镜像队列
docker exec rabbitmq-node1 rabbitmqadmin declare queue \
  name=mirrored.order.pending \
  durable=true 2>/dev/null || true

docker exec rabbitmq-node1 rabbitmqadmin declare queue \
  name=mirrored.order.confirmed \
  durable=true 2>/dev/null || true

# 创建一个普通队列（非镜像）作为对比
docker exec rabbitmq-node1 rabbitmqadmin declare queue \
  name=normal.order.log \
  durable=true 2>/dev/null || true

echo "  ✓ 队列已创建"
echo ""
echo "  队列分布:"
docker exec rabbitmq-node1 rabbitmqctl list_queues name node policy type --formatter=table 2>/dev/null | head -20
...
MASTER_NODE=$(docker exec rabbitmq-node1 rabbitmqctl list_queues name node --formatter=csv 2>/dev/null | grep "mirrored.order.pending" | cut -d',' -f2 | tr -d '"')
echo "  mirrored.order.pending 的 master 在: ${MASTER_NODE}"

# 提取节点序号
MASTER_IDX=$(echo ${MASTER_NODE} | grep -oP 'node\K\d')
...
echo "  master: ${MASTER_NODE} (节点${MASTER_IDX})"
echo "  mirror: ${SLAVE_NODE} (节点${SLAVE_IDX})"

# ---------- 6. 模拟 master 宕机，观察故障切换 ----------
echo ""
echo "[6/6] ====== 模拟故障切换 ======"
echo ""
echo "  ▸ 即将停止 master 节点: ${MASTER_NODE}"
echo "  ▸ 预期: ${SLAVE_NODE} 上的 mirror 自动提升为新 master"
echo "  ▸ 队列 mirrored.order.pending 继续可用"
echo ""
echo "  执行: docker stop ${MASTER_NODE}"
echo ""
...
echo ""
echo "  观察集群状态 (从 node${SLAVE_IDX} 查看):"
docker exec ${SLAVE_NODE} rabbitmqctl cluster_status 2>/dev/null | grep -A5 "Running Nodes" || echo "  (集群状态查询可能短暂失败，正在恢复中...)"
echo ""

# 验证队列仍可用
echo "  验证队列 mirrored.order.pending 是否仍可访问..."
sleep 2
if docker exec ${SLAVE_NODE} rabbitmqadmin get queue=mirrored.order.pending ackmode=ack_requeue_false 2>/dev/null | grep -q "orderId"; then
  echo "  ✓ 故障切换成功! 队列仍可消费，消息未丢失"
else
  echo "  (正在选举新 master，稍后重试...)"
  sleep 5
  docker exec ${SLAVE_NODE} rabbitmqadmin get queue=mirrored.order.pending ackmode=ack_requeue_false 2>/dev/null | head -5
fi
...
echo ""
echo " 恢复 master 节点:"
echo "   docker start ${MASTER_NODE}"
echo "   docker exec ${MASTER_NODE} rabbitmqctl start_app"
echo ""
echo " 观察点:"
